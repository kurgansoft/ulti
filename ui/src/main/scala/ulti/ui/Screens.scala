...
object Screens {
  ...
  def spectatorScreen(spectatorState: SpectatorState, eventHandler: EventLoop.EventHandler[Event]): TagOf[Div] = {
...
//              (1900, 1050)
  ...
  def playerScreen(clientState: ClientState, eventHandler: EventLoop.EventHandler[Event]): TagOf[Div] = {
    Directives.loadExternalCss()
      ...
      if (ultiPlayer.isDefined) {
        ...
        offlineUltiState.currentSubScreen match {
          case Some(LeftPlayerPerspectiveScreen) => {
            SubScreens.foreignPlayerPerspective(innerUlti, playerToTheLeft, eventHandler, offlineUltiState.smallCards)(fu.players)
          }
          case Some(RightPlayerPerspectiveScreen) => {
            SubScreens.foreignPlayerPerspective(innerUlti, playerToTheRight, eventHandler, offlineUltiState.smallCards)(fu.players)
          }
          case Some(ContractInfoScreen) => {
            SubScreens.contractsInfoSubScreen(innerUlti, ultiPlayer.get, eventHandler)(fu.players)
          }
          case Some(BidSelectionScreen) => {
            div(color:="yellow",
              Directives.bidPropositionScreen(offlineUltiState.bidProposition, innerUlti.winningBidTuple, eventHandler, true)(fu.players)
              ...
              button(`class`:="btn btn-primary", "BACK TO THE MAIN SCREEN", onClick --> Callback {
                eventHandler(SwitchToSubScreen(None))
              })(position:="relative", left:="50%", transform:="translate(-50%)")
            )
          }
          case Some(DoublingScreen) => {
            div(color:="yellow",
              Directives.doublingDirective(innerUlti.currentPlayer, innerUlti.contractsThatCanBeDoubledByPlayer(ultiPlayer.get), eventHandler),br,br,
          ...
          case None => {
            ...
            div(color:="yellow",
              div(
                Directives.cardHand(scw, eventHandler = eventHandler, radius = customRadius, centerPoint = cardHandCenterPoint),
              ...
              Directives.stuffInTheMiddle(offlineUltiState, ulti, ultiPlayer.get, eventHandler, offlineUltiState.smallCards)(clientState.frontendUniverse.get.players),
              Option.when(ultiPlayer.contains(innerUlti.currentPlayer) && innerUlti.phase == BiddingPhase && !innerUlti.talonOnTheTable)(
                div(position:="relative",
                  if (offlineUltiState.separateBidScreen) {
                    div(
                      button("TAP to specify your bid!", `class`:="btn btn-primary", position:="relative", left:="50%", transform:="translate(-50%)", onClick --> Callback {
                        eventHandler(SwitchToSubScreen(Some(BidSelectionScreen)))
                      }),
                  ...
                  div(
                    Directives.submitBidButton(offlineUltiState, innerUlti.currentBid, offlineUltiState.bidProposition.generatedBid, eventHandler)
                    (position:="relative", left:="50%", transform:="translate(-50%)")
                  )
                )
              ),
              ...
              Directives.clientLeftCorner(innerUlti, eventHandler)(position:="absolute", bottom:="0px", left:="5px"),
              ...
              ({
                if (innerUlti.currentBidWinner.contains(ultiPlayer.get)) {
                  div(
                    Directives.doublingDirective(innerUlti.currentPlayer, innerUlti.contractsThatCanBeDoubledByPlayer(ultiPlayer.get), eventHandler), br,
                    button(`class`:="btn btn-primary", "DONE WITH DOUBLING", onClick --> Callback {
                      eventHandler(DoneWithDoublingEvent)
                    })
                  ...
                  div(
                    button("DOUBLE", `class`:="btn btn-primary", onClick --> Callback {
                      eventHandler(SwitchToSubScreen(Some(DoublingScreen)))
                    } )
              ...
              div(
                ...
                (button(`class`:="btn btn-primary", "SHOW PERSPECTIVE", onClick --> Callback {
                  eventHandler(SwitchToSubScreen(Some(RightPlayerPerspectiveScreen)))
                })),
              ...
              div(
                Option.when(innerUlti.phase == PlayingPhase && innerUlti.round > 1 && innerUlti.currentBid.exists(_.open))
                (button(`class` := "btn btn-primary", "SHOW PERSPECTIVE", onClick --> Callback {
                  eventHandler(SwitchToSubScreen(Some(LeftPlayerPerspectiveScreen)))
                })),
  ...
  private def calculateCardCoordinates(degree: Double): (Int, Int) = {
  ...
  private def table(ulti: ClientUlti, w: Int, h: Int)(implicit players: List[FrontendPlayer]): TagOf[Div] = {
      ...
      div(
        div(
          innerUlti.getSortedPlayerHand(UltiPlayer1) match {
            ...
            case Right(cards) => Directives.cardHand(cards.map(card => (card, false)), radius = customRadius, centerPoint = (horizontalOffset2/2, -100), eventHandler = null)
        ...
        div(
          innerUlti.getSortedPlayerHand(UltiPlayer2) match {
            ...
            case Right(cards) => Directives.cardHand(cards.map(card => (card, false)), radius = customRadius, borderDegree = 135, centerPoint = (horizontalOffset2/2, 0), eventHandler = null)
        ...
        div(
          innerUlti.getSortedPlayerHand(UltiPlayer3) match {
            ...
            case Right(cards) => Directives.cardHand(cards.map(card => (card, false)), radius = customRadius, borderDegree = 45, centerPoint = (horizontalOffset2/2, 0), eventHandler = null)
  ...
  private def otherStuff(ulti: ClientUlti)(implicit players: List[FrontendPlayer]): TagOf[Div] = {
    val innerUlti = ulti.innerUlti.get
    div(color:="yellow",
