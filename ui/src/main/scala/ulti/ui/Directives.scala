...
object Directives {
  ...
  def calculateCardCoordinates(centerPoint: (Int,Int), radius: Int, degree: Double, selected: Boolean = false): (Int, Int) = {
    val extendedRadius: Double = if (selected) radius * 1.2 else radius
  ...
  def stuffInTheMiddle(offlineUltiState: OfflineUltiState,
                       ulti: ClientUlti,
                       you: UltiPlayer, 
                       eventHandler: EventLoop.EventHandler[OfflineUltiEvent],
                       smallCards: Boolean = false)(
                          implicit players: List[FrontendPlayer]
                       ): TagOf[Div] = {
    // bidding Phase
    ...
    innerUlti.phase match {
        ...
        if (innerUlti.talonOnTheTable) {
          div(
            if (innerUlti.currentPlayer == you) {
              pickUpTalonScreen(innerUlti.currentBidWinner.contains(innerUlti.currentPlayer), eventHandler, offlineUltiState.smallCards)
            } else {
      ...
      case PlayingPhase =>
        div(
            ...
            (Directives.declareTrumpSuitScreen(innerUlti.currentBid.get, eventHandler)(position:="relative", left:="50%", transform:="translate(-50%)", width:="fit-content")),
            Option.when(marriageScreenShouldBeDisplayed(offlineUltiState.marriagesAreDeclared, innerUlti, you))
            (Directives.declareMarriagesScreen(offlineUltiState.proposedMarriages, innerUlti, you, eventHandler)
            (position:="relative", left:="50%", transform:="translate(-50%)", width:="fit-content")),
  ...
  def animatedCardsInTheMiddle(cards: List[Card], animationDirection: Option[Int] = None, smallCards: Boolean = false): TagOf[Div] = {
    ...
    val base = div(position:="absolute", bottom:="0px", left:="0px", zIndex:="-999",
      Directives.cardHand(cards.map((_, false)), -90, (x, y), radius = customRadius,eventHandler = null),
    )
  ...
  def cardHand(cards: List[(Card, Boolean)], borderDegree: Int = -90, centerPoint: (Int, Int) = (960 - (178/2), -100), radius: Int = 300, eventHandler: EventLoop.EventHandler[CardClicked]): TagMod = {
      ...
      (position:="absolute", bottom:=y+"px", left:=x+"px", transform:= "rotate(" + (rotationNumber - 90) + "deg)",
        onClick --> Callback {
          if (eventHandler != null)
            eventHandler(CardClicked(card._1))
        }
      ))
    }).toTagMod
  }
  ...
  def cardsOnTheTable(cards: List[Card]): TagOf[Div] = {
    div(
      cardHand(cards.map((_, false)), -90, (960 - (178/2), 250), eventHandler = null)
    )
  }
  ...
  def bidPropositionScreen(bp: BidProposition, 
                           currentBidWinnerAndBid: Option[(UltiPlayer, Bid)] = None,
                           commander: EventLoop.EventHandler[BidPropositionEvent],
                           smallCards: Boolean = false)(
                              implicit players: List[FrontendPlayer]): TagOf[Div] = {
  ...
  def submitBidButton(
                       ...
                       commander: EventLoop.EventHandler[SubmitBidEvent.type]): TagOf[Button] = {
    ...
    button(`class`:="btn btn-primary", "SUBMIT BID",
      onClick --> Callback {
        commander(SubmitBidEvent)
      },
      disabled:=(!buttonEnabled)
    )
  }

  def pickUpTalonScreen(startPlayingPhase: Boolean, commander: EventLoop.EventHandler[OfflineUltiEvent], smallCards: Boolean = false): TagOf[Div] = {
    ...
    div(display:="flex", flexDirection:="row", alignItems:="center",
      button(`class`:="btn btn-primary", "Pick up Talon", width:="121px", position:="relative", top:="-25px",
        onClick --> Callback {
          commander(PickUpTalonEvent)
        }
      ),
      Directives.talon(smallCards),
      button(`class`:="btn btn-primary", skipText, width:="121px", position:="relative", top:="-25px",
        onClick --> Callback {
          commander(SkipBidEvent)
        }
      )
    )
  }

  def declareTrumpSuitScreen(bid: Bid, commander: EventLoop.EventHandler[OfflineUltiEvent]): TagOf[Div] = {
    div(marginTop:="25px",
      ...
      (button(`class`:="btn btn-primary", "Plain", height:="fit-content", width:="80px", textAlign:="center", marginTop:="15px",
        onClick --> Callback {
          commander(DeclareTrumpSuitEvent(Plain))
        }, position:="relative", left:="50%", transform:="translate(-50%)"
      ))
    )
  }

  def declareMarriagesScreen(proposedMarriages: Set[CardSuit], innerUlti: ClientInnerUlti, player: UltiPlayer, commander: EventLoop.EventHandler[OfflineUltiEvent]): TagOf[Div] = {
    ...
    def callbackForSuit(suit: CardSuit): ReactEventFrom[Input] => Callback = event => Callback {
      ...
      commander(AddOrRemoveMarriageEvent(suit))
    }
    ...
    div(
      ...
      button(`class`:="btn btn-primary", "Declare the selected marriages",
        onClick --> Callback {
          commander(DeclareMarriagesEvent)
        }
      )
    )
  }
  ...
  def resultsDirective(innerUlti: ClientInnerUlti)(implicit players: List[FrontendPlayer]): TagOf[Div] = {
    ...
    val theList: List[TagOf[Div]] = playerContractPairs.filterNot(_._2.contractResult == Open).flatMap(t => {
      ...
      List(
        div(contract.toString + " (" + contractor + ")"),
        div(result.toString),
  ...
  def doublingDirective(currentPlayer: UltiPlayer, contractsToDouble: List[ContractWithStatus], commander: EventLoop.EventHandler[DoubleContractEvent]): TagOf[Div] = {
    div(display:="flex", flexDirection:="column",
      contractsToDouble.map(contractWithStatus => {
        ...
        button(`class`:="btn btn-primary", doublingText + " " + contractText, onClick --> Callback {
          commander(DoubleContractEvent(contractWithStatus.contract))
        }, marginBottom:="20px")
      }).toTagMod
    )
  }

  def clientLeftCorner(innerUlti: ClientInnerUlti, commander: EventLoop.EventHandler[SwitchToSubScreen]): TagOf[Div] = {
    innerUlti.phase match {
      case BiddingPhase => div("Bidding Phase")
      case PlayingPhase =>
        div(
          button("BID INFO", `class`:="btn btn-primary", onClick --> Callback {
            commander(SwitchToSubScreen(Some(ContractInfoScreen)))
          }),
          div(trumpSuitText(innerUlti.trumpSuit))
        )
