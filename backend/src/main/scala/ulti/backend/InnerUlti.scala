...
case class InnerUlti(
  ...
  implicit def convert (x: InnerUlti): Either[Failure, InnerUlti] = Right(x)
  ...
  private lazy val resolveOpenContracts: InnerUlti = {
  ...
  // (InnerUlti, IO[Failure, Seq[Action]])]
  def reduce(ultiAction: UltiAction, invoker: UltiPlayer): Either[Failure, InnerUlti] = {
    ultiAction match {
      ...
      case _ => Left(GeneralFailure(s"Action $ultiAction is not implemented."))
    }
  }

  private def reduceSubmitBid(bid: Bid, talon: Set[Card], invoker: UltiPlayer): Either[Failure, InnerUlti] = {
    if (phase != BiddingPhase) {
      Left(GeneralFailure("Wrong phase."))
    } else if (invoker != currentPlayer) {
      Left(GeneralFailure("Not your turn."))
    } else if (this.playerHands(currentPlayer).size != 12) {
      Left(GeneralFailure("You can only bid if the talon is in your hand."))
    } else if (!talon.subsetOf(playerHands(currentPlayer))) {
      Left(GeneralFailure("You don't have those cards."))
      ...
      Left(GeneralFailure("Your bid must be higher than the current one."))
    }
  }

  private def reducePickUpTalon(invoker: UltiPlayer): Either[Failure, InnerUlti] = {
    if (phase != BiddingPhase) {
      Left(GeneralFailure("Wrong phase."))
    } else if (!talonOnTheTable) {
      Left(GeneralFailure("There is no talon to pick up."))
    } else if (currentPlayer!=invoker) {
      Left(GeneralFailure("Not your turn."))
    } else {
      this.copy(talonOnTheTable = false, playerHands = playerHandLen(currentPlayer).modify(_ ++ talon)(playerHands))
    }
  }

  private def reduceSkipBid(invoker: UltiPlayer): Either[Failure, InnerUlti] = {
    if (phase != BiddingPhase) {
      Left(GeneralFailure("Wrong phase."))
    } else if (currentPlayer != invoker) {
      Left(GeneralFailure("Not your turn."))
    } else  if (!talonOnTheTable) {
      Left(GeneralFailure("You cannot skip the bid, since the talon is in your hand."))
    } else {
      if (currentBidWinner.contains(currentPlayer)) {
        this.transitionToPlayingPhase
      } else {
        this.copy(currentPlayer = currentPlayer.nextPlayer, round = newRoundNumber)
      }
    }
  }

  private def reduceDeclareTrumpSuit(suit: CardSuit, invoker: UltiPlayer): Either[Failure, InnerUlti] = {
    if (phase != PlayingPhase || round != 1) {
      Left(GeneralFailure("Declaring the trump suit is only possible in the first round of the playing phase."))
    } else if (!currentBidWinner.contains(currentPlayer) || invoker != currentPlayer) {
      Left(GeneralFailure("Only the bid winner can declare the trump suite."))
    } else if (trumpSuit.isDefined) {
      Left(GeneralFailure("Trump suite has been already declared."))
    } else if (!currentBid.get.trumpGame) {
      Left(GeneralFailure("This is a plain game, trump suite cannot be declared."))
    } else {
  ...
  private def reduceDeclareMarriages(suits: Set[CardSuit], invoker: UltiPlayer): Either[Failure, InnerUlti] = {
    if (phase != PlayingPhase || round != 1) {
      Left(GeneralFailure("Declaring marriages is only possible in the first round of the playing phase."))
    } else if (invoker != currentPlayer) {
      Left(GeneralFailure("You can only declare marriages in your turn."))
    } else if (declaredMarriages.keys.toList.contains(currentPlayer)) {
      Left(GeneralFailure("Marriages are declared already."))
    } else if (!currentBid.get.canMarriagesBeDeclared) {
      Left(GeneralFailure("Marriages can't be declared for this game."))
    } else {
      if (suits.forall(doesPlayerHaveAMarriageInSuit(invoker, _))) {
        this.copy(declaredMarriages = declaredMarriageLen(currentPlayer).replace(suits)(declaredMarriages))
      } else {
        Left(GeneralFailure("One or more marriage that you are trying to declare is missing."))
      }
    }
  }

  private def reducePlayCard(card: Card, invoker: UltiPlayer): Either[Failure, InnerUlti] = {
    if (phase != PlayingPhase) {
      Left(GeneralFailure("Cards can only be played during the playing phase"))
    } else if (invoker != currentPlayer) {
      Left(GeneralFailure("You can only play a card in your turn."))
    } else if (!playerHands(currentPlayer).contains(card)) {
      Left(GeneralFailure("You don't have that card in your hand."))
    } else if (round == 1 && trumpSuit.isEmpty){
      Left(GeneralFailure("Trump suit has to be declared before playing a card."))
    } else {
      implicit val adu: CardSuit = trumpSuit.get
      val validationResult = validMove(card)
      if (validationResult.nonEmpty) {
        Left(GeneralFailure(validationResult.get))
      } else {
        ...
        cardsOnTheTable.size match {
          case 2 =>
            ...
            if (temp2.contracts.values.forall(_.forall(contractWithStatus => {
              ...
              temp2.copy(phase = GameOverPhase, round = nr)
            } else if (temp2.round == 11)
              temp2.copy(phase = GameOverPhase, round = 10)
            else
              temp2.copy(currentPlayer = roundWinner)
          case _ =>
            temp.copy(currentPlayer = temp.currentPlayer.nextPlayer)
        }
      }
    }
  }

  private def reduceDoubleContract(contract: Contract, invoker: UltiPlayer): Either[Failure, InnerUlti] = {
    if (phase != PlayingPhase || round != 1)
      Left(GeneralFailure(InnerUlti.FailureMessages.doublingOutsideTheFirstRound))
    else {
      val cws = contracts(currentBidWinner.get).find(_.contract == contract)
      if (cws.isEmpty) {
        Left(GeneralFailure(InnerUlti.FailureMessages.doublingNonExistentContract))
      } else {
        if (contract.silent) {
          Left(GeneralFailure(InnerUlti.FailureMessages.doublingSilentContract))
        } else {
          val currentLevel = cws.get.contraLevel.levelForPlayer(currentPlayer)
          if (currentLevel >= ContraLevel.maximumContraLevel) {
            Left(GeneralFailure(InnerUlti.FailureMessages.maximumContraLevelAlreadyReached))
          } else if (currentLevel % 2 == 0) { // defenders turn
            if (invoker == currentBidWinner.get) {
              Left(GeneralFailure(InnerUlti.FailureMessages.defendersTurnToDouble))
            } else if (invoker != currentPlayer) {
              Left(GeneralFailure(InnerUlti.FailureMessages.defendersTurnButNotYourTurn))
            } else {
              ...
              })(this.contracts), blockIsLifted = false)
            }
          } else { // soloist turn
            if (invoker != currentBidWinner.get) {
              Left(GeneralFailure(InnerUlti.FailureMessages.soloistTurnToDouble))
            } else {
              if (blockIsLifted) {
                Left(GeneralFailure(InnerUlti.FailureMessages.soloistHasLiftedTheBlockAlready))
              } else {
                this.copy(contracts = contractsOfTheBidWinnerLen.modify(old => {
                  val contractToDouble = old.find(_.contract == contract).get
                  val doubledContract = contractToDouble.doubleForPlayer(currentPlayer, currentBidWinner.get)
  ...
  private def reduceDoneWithDoubling(invoker: UltiPlayer): Either[Failure, InnerUlti] = {
